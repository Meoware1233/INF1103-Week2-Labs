...
def save_inventory(inventory, history):
    with open("inventory.txt", "w") as file:
        ...
        for i in history:
            file.write(str(i) + "\n")
    print("Inventory sucessfully saved into inventory.txt")

def get_valid_input(inventory, history):
    rejected_entries = 0
    while True:
        if inventory > 500:
            print("Inventory warning, inventory has exceeded 500 units. Exiting program.")
            return inventory, history ,rejected_entries
        else:
            ...
            if stock_quantity_input == 'quit':
                ...
                try: 
                    if int(stock_quantity_input) < 0:
                        ...
                        history.append(int(stock_quantity_input))
...
exist_inventory, exist_history = load_inventory()
inventory, history, rejected_entries = get_valid_input(exist_inventory, exist_history)
